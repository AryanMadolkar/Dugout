...
export function CaptainPanel() {
  ...
  const pick = captain ?? alternatives[0];
        ...
        >
          ...
          <div className="mt-3 grid grid-cols-2 gap-2">
            <div className="rounded-[3px] bg-white/80 px-2 py-1.5 text-center">
              ...
              <p className="text-[13px] font-extrabold">{pick.form.toFixed(1)}</p>
            </div>
            <div className="rounded-[3px] bg-white/80 px-2 py-1.5 text-center">
          ...
          <p className="mt-3 text-right text-[24px] font-extrabold text-[var(--positive)]">{pick.xp.toFixed(1)} xP</p>
        </div>

        {alternatives.filter((a) => a.id !== pick.id).length > 0 ? (
          <>
            <p className="font-label mb-2 mt-4 text-[11px] text-[var(--text-secondary)]">Alternatives</p>
            <div className="space-y-2">
              {alternatives
                .filter((a) => a.id !== pick.id)
                .map((alt, i) => (
                  <div
                    key={alt.id}
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <p className="text-[13px] font-bold">{alt.name}</p>
                        <p className="font-label text-[10px] text-[var(--text-secondary)]">{alt.club}</p>
                      </div>
                    </div>
