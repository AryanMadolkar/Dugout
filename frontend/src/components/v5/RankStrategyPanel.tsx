...
export function RankStrategyPanel() {
  const { starters, hasSquad, strategyMode, fplRank, setFplRank } = useDashboard();
        ...
        <div className="flex flex-wrap items-end gap-3">
          <div>
            ...
            <input
              ...
              value={fplRank ?? ""}
              placeholder="412806"
              onChange={(e) => setFplRank(e.target.value ? Number(e.target.value) : null)}
              className="control mt-1 w-32 border border-[var(--border)] px-2 py-1 text-[14px] font-bold"
            />
          </div>
          <div>
